...
// 04 "created" LONGDATE CS_LONGDATE NOT NULL , 
    ...
    pStmt = param.connection.prepareStatement('select "IOT"."xsc-iot.data::tempId".NEXTVAL from dummy');
    ...
    for (var i=0; i<2; i++) {
        //var pStmt;
        //insert into "IOT"."xsc-iot.data::sensors.temp" values("xsc-iot.data::tempId".NEXTVAL,100,TO_TIMESTAMP ('2016-02-02 13:23:45.678', 'YYYY-MM-DD HH24:MI:SS.FF3'),CURRENT_UTCTIMESTAMP);

        if (i<1) {
            pStmt = param.connection.prepareStatement('insert into "IOT"."xsc-iot.data::sensors.temp" values(?,?,TO_TIMESTAMP (?, \'YYYY-MM-DD HH24:MI:SS.FF3\'),CURRENT_UTCTIMESTAMP)');
        }
        else {
